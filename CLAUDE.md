...
<Prompt name="Ryoma_AI_Daily_v7" version="2.0" lang="ja">
  ...
  <structure>
    <opening_lead>
      タイトル直後に「読みたくなる訴求文」を配置（2-4文、100-200字）。
      - FOMO/損失回避を訴求（「今読まないと○○で損する」）
      - 記事の核心的なベネフィットを数値で提示
      - 読者の痛みに共感しつつ、解決策があることを示唆
      - H2見出しは付けず、本文としてそのまま配置
    </opening_lead>
  ...
  <process>
    ...
    <step>タイトル直後のリード文作成（2-4文、100-200字）→FOMO訴求＋数値ベネフィット＋読者共感</step>
  ...
  <final_output>
    ...
    <blog_body>
      タイトル直後にリード文（2-4文、100-200字、H2見出しなし）を配置。
      続いてH2セクションを最低6つ以上＋最後に「まとめ（PREP）」を配置。
    </blog_body>
...
### 基本方針
修正依頼があった場合、フィードバックを元に抽象化した学びをCLAUDE.mdの執筆ルールに反映してください。

### 作業フロー
1. **フィードバック受領**: ユーザーから構成・内容・表現に関する修正依頼を受け取る
2. **抽象化**: 個別の修正内容から、再現可能な一般的なルールを抽出
3. **CLAUDE.md更新**: 該当するプロンプトセクションに新しいルールを反映
4. **検証**: 更新したルールが他の記事作成にも適用可能か確認

### 抽象化の例
- 「今日のハイライト不要」→「タイトル直後にH2見出しなしのリード文を配置」
- 「もっと具体的な数値を」→「各セクションに最低1つの定量データを含める」
- 「専門用語が難しい」→「専門用語には必ず［やさしい補足］を付ける」

### 更新対象セクション
- `<structure>`: 記事構成に関するフィードバック
- `<title_rules>`: タイトルに関するフィードバック
- `<sns_hooks>`: SNS投稿文に関するフィードバック
- `<quality_checks>`: 品質基準に関するフィードバック
- その他該当セクション

この継続的改善プロセスにより、プロンプトは実際の執筆経験から学習し、より精度の高い記事を生成できるようになります。
